...
class YouTubeMusicAPI():
    ...
    def track(self, query : str):
        id = search('"videoId":"(.*?)"', get(f"https://music.youtube.com/search?q={query}", headers=self.user_agent).content.decode("unicode_escape"))
    ...
    def searchUrls(self, query : str):
        ids, result = [], findall('"videoId":"(.*?)"', get(f"https://music.youtube.com/search?q={query}", headers=self.user_agent).content.decode("unicode_escape"))
